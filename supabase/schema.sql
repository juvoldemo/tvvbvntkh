...
begin
  if not exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'Allow anonymous page view inserts'
  ) then
...
revoke all on table app_analytics_events from anon, authenticated;
create index if not exists idx_app_analytics_created_at on app_analytics_events(created_at desc);
create index if not exists idx_app_analytics_advisor on app_analytics_events(advisor_code, created_at desc);
create index if not exists idx_app_analytics_session on app_analytics_events(session_id);
...
create table if not exists tvv_target_registrations (
  id uuid primary key default gen_random_uuid(),
  target_month date not null,
  advisor_code text not null,
  advisor_name text,
