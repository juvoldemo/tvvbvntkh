(function () {
  ...
  function hasMojibakeText(value) {
    return /[\u00c3\u00c4\u00c2\u00c1\u00e1\u00ba\u00bb\u00bb\u00f0\u00e2\u0080-\u009f\u0192\u2020\u2021\u2018-\u201d\u2022\u2026\u2030\u2039\u203a\u0152\u0153\u0178\u0160\u0161\u017d\u017e]/.test(String(value ?? ""));
  }

  function normalizeText(value) {
    const textValue = String(value ?? "");
    if (!hasMojibakeText(textValue)) return textValue;
    const cp1252 = {
    ...
    const toByte = (char) => cp1252[char.charCodeAt(0)] ?? (char.charCodeAt(0) & 0xff);
    ...
    try {
      return decodeSegment(textValue);
    } catch (error) {
      // Continue with partial recovery below.
    }

    const decodedWords = textValue.split(/([ \t\r\n]+)/).map((token) => {
    ...
    if (decodedWords !== textValue) return decodedWords;

    return textValue.replace(/[A-Za-z\u00c0-\u1ef90-9%.,:/()\-+\s\u0080-\u017f\u2010-\u2122]+/g, (segment) => {
  ...
  function normalizePlan(plan) {
    ...
    return PLAN_INDEX[normalizedPlan] === undefined ? "V\u00e0ng" : normalizedPlan;
  }
  ...
  function buildAddonCards(data, config) {
    return (data.selectedAddons || []).map((addon) => {
      ...
      return {
        code: addon.code,
        name: addon.name,
        personLabel: addon.personLabel || (addon.personKey === "policyOwner" ? "BMBH" : "N\u0110BH"),
        personName: addon.personName || data.insured?.name || "",
        headline,
  ...
  function drawPersonBadge(ctx, card, x, y, maxWidth) {
    const isPolicyOwner = card.personLabel === "BMBH" || card.personKey === "policyOwner";
    const bg = isPolicyOwner ? "#fff4d8" : "#eaf5ff";
    const fg = isPolicyOwner ? "#c98200" : COLORS.blue;
    const label = `${card.personLabel || "N\u0110BH"}: ${card.personName || "-"}`;
    const width = Math.min(maxWidth, Math.max(210, ctx.measureText(normalizeText(label)).width + 34));
    ctx.fillStyle = bg;
    drawRoundedRect(ctx, x, y, width, 30, 15);
    ctx.fill();
    text(ctx, label, x + 17, y + 6, { size: 17, weight: 900, color: fg, maxWidth: width - 34 });
    return width;
  }

  function estimateCardHeight(card, layout) {
    return layout.cardHeaderH + 34 + layout.tableHeaderH + card.rows.length * layout.rowH + layout.cardPadding * 2;
  }
  ...
  function drawCard(ctx, card, x, y, width, layout) {
    ...
    drawPersonBadge(ctx, card, x + layout.cardPadding + 88, y + layout.cardPadding + layout.titleSize + layout.bodySize + 13, 360);

    const tableX = x + layout.cardPadding;
    const tableY = y + layout.cardPadding + layout.cardHeaderH + 34;
  ...
  function estimateInfographicCardHeight(card) {
    ...
    if (card.code === "R21" && !card.continued) {
      ...
      return 112 + topH + 12 + groupMiniH + 44;
    }
    return 112 + gridHeight(card, card.rows, innerW) + 34;
  }
