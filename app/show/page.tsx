...
type Advisor = { advisorCode: string; advisorName: string; revenueTarget: number; currentRevenue: number };
...
function millionNumber(value: number) { return (Number(value || 0) / 1_000_000).toLocaleString("vi-VN", { maximumFractionDigits: 1 }); }
...
export default function TargetShowPage() {
  ...
  return <main className="target-show-page">
    <header className="target-show-header"><div><span><Target size={28} /></span><div><h1>Thần giao cách cảm</h1><p>Tháng {data ? `${data.month.slice(5, 7)}/${data.month.slice(0, 4)}` : "--/----"}</p></div></div><aside><small><i className="target-show-live-dot" />{realtimeStatus}</small><button type="button" aria-label="Reset dữ liệu" title="Reset dữ liệu" onClick={() => { setResetError(""); setResetOpen(true); }}><RefreshCcw size={18} /></button></aside></header>
    <nav className="target-show-tabs" aria-label="Chế độ xem"><button className={view === "advisor" ? "active" : ""} onClick={() => setView("advisor")}><UserRound size={17} />TVV đăng ký</button><button className={view === "leader" ? "active" : ""} onClick={() => requestProtectedView("leader")}><Users size={17} />Trưởng nhóm</button><button className={view === "comparison" ? "active" : ""} onClick={() => setView("comparison")}><GitCompareArrows size={17} />So sánh</button></nav>
    ...
    {view !== "comparison" && <section className="target-show-groups">{groups.map((group, index) => <article className={`target-show-group${index === 0 && group.revenueTarget > 0 ? " is-leader" : ""}`} style={{ viewTransitionName: transitionName(`${view}-${group.groupName}`) }} key={group.groupName}><header><i>{index + 1}</i><div><h2>{group.groupName}</h2><small className="target-show-leader">{group.leaderName}</small><p><b>{compactMoney(group.revenueTarget)}</b><span>{group.advisorCount} TVV</span></p></div></header><div className="target-show-advisors">{group.advisors.map((advisor) => <div key={advisor.advisorCode || advisor.advisorName}><span><b>{advisor.advisorName}</b></span><strong><em>{millionNumber(advisor.currentRevenue)}</em><i>/</i>{millionNumber(advisor.revenueTarget)} tr</strong></div>)}{!group.advisors.length && <p>Chưa có TVV đăng ký</p>}</div></article>)}</section>}
