...
import { isGroupInManagedBoard } from "@/lib/board-scope";
...
export default function TvvMobilePage() {
  ...
  const showCustomerInvitation = (
    userProfile?.dashboard_role === "advisor" || userProfile?.dashboard_role === "team_leader"
  ) && isGroupInManagedBoard(
    userProfile?.dashboard_role === "team_leader" ? userProfile?.managed_group_name : userProfile?.group_name,
    "Huỳnh Thị Vân Anh"
  );
  ...
  return (
    <main className={`tvv-app${isAdoMode ? " ado-app" : ""}`}>
      ...
      {illustrationLoaded && <IllustrationTab active={tab === "illustration"} premiumText={illustrationPremiumText} advisorCode={userProfile?.advisor_code} />}
...
function TeamLeaderOverview({ advisorCode, data, targetRegistration, targetMonth, targetRegistrationClosed, teamGoalDetailSignal, onOpenTarget, contestEstimate, currentTeamAdvisorCount, leaderboard, month, monthOptions, onMonthChange, onOpenLeaderboard, onOpenContests, onOpenRecruitment, onOpenSmart, onOpenAbout, showCustomerInvitation }: any) {
  ...
  return <section className="tvv-content team-dashboard">
    ...
    {showCustomerInvitation && <GuestInvitationHomeCard />}
...
function Overview({ advisorCode, showRecruitment, showCustomerInvitation, stats, leaderboard, estimate, starViet, starVietWarning, onTab }: any) {
  const statItems = [
...
function IllustrationTab({ active, premiumText = "", advisorCode = "" }: { active: boolean; premiumText?: string; advisorCode?: string }) {
  const minhHoaVersion = "20260808-advice-export";
  const src = `/minhhoa2/index.html?embedded=1&v=${minhHoaVersion}${premiumText ? `&annualPremium=${encodeURIComponent(premiumText)}` : ""}${advisorCode ? `&advisorCode=${encodeURIComponent(advisorCode)}` : ""}`;
  return (
