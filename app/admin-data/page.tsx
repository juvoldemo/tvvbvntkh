...
function AnalyticsPanel({ period, setPeriod, data, loading, onReload }: { period: AnalyticsPeriod; setPeriod: (value: AnalyticsPeriod) => void; data: AnalyticsData | null; loading: boolean; onReload: () => void }) {
  ...
  return <article className="admin-card admin-analytics-card">
    ...
    {showInvitationRanking && <div className="admin-analytics-modal-backdrop" onClick={() => setShowInvitationRanking(false)}><section className="admin-analytics-modal admin-invitation-ranking-modal" role="dialog" aria-modal="true" aria-labelledby="invitation-ranking-title" onClick={(event) => event.stopPropagation()}><header><div><h3 id="invitation-ranking-title">Thư mời theo TVV</h3><p>{summary.invitationsCreated} thư mời được tạo từ 01/08 · xếp theo số lượng giảm dần</p></div><button type="button" onClick={() => setShowInvitationRanking(false)} aria-label="Đóng"><X size={22} /></button></header><div className="admin-invitation-ranking-list">{(data?.invitationRanking ?? []).map((item, index) => <div key={item.advisorCode}><b className={`admin-rank-number rank-${index + 1}`}>{index + 1}</b><span><strong>{item.fullName}</strong><small>{item.advisorCode} · {item.groupName}{item.position !== "—" ? ` · ${item.position}` : ""}</small></span><em>{item.count}<small> thư</small></em></div>)}{!(data?.invitationRanking ?? []).length && <p>Chưa có dữ liệu tạo thư mời từ 01/08.</p>}</div></section></div>}
    {!showSessionDetails && !showAccessRanking && <><div className="admin-analytics-summary admin-analytics-summary-full">
      <span><strong>{summary.uniqueAdvisors}</strong> TVV truy cập</span><span><strong>{summary.sessions}</strong> phiên truy cập</span><span><strong>{summary.actions}</strong> thao tác</span><button type="button" className="admin-analytics-summary-action" onClick={() => setShowInvitationRanking(true)} aria-haspopup="dialog"><strong>{summary.invitationsCreated}</strong> thư mời từ 01/08</button><span><strong>{duration(summary.averageSeconds)}</strong> trung bình/phiên</span><span><strong>{summary.summaryExports}</strong> lượt xuất tóm tắt</span><span><strong>{summary.viewOnlySessions}</strong> phiên chỉ xem</span>
    </div>
